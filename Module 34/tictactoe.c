#include<stdio.h>
#include<stdbool.h>

int isWin(int a[][4],int n)
{
    for(int i=1; i<=n; i++)
    {
        if(a[i][1] == a[i][2] && a[i][2]==a[i][3] && a[i][1]!=-1)
        {
            return a[i][1];
        }
    }
    for(int j=1; j<=n; j++)
    {
        if(a[1][j]==a[2][j] && a[2][j]==a[3][j] && a[1][j]!=-1)
        {
            return a[1][j];
        }
    }

    if(a[1][1]==a[2][2] && a[2][2]==a[3][3] && a[1][1]!=-1)
    {
        return a[1][1];
    }
    else if(a[1][3]==a[2][2] && a[2][2]==a[3][1] && a[1][3]!=-1)
    {
        return a[1][3];
    }

    return -1;
}
void print_cells(int a[][4],int n)
{
    printf("\n");
    for(int i=1; i<=n; i++)
    {
        for(int j=1; j<=n; j++)
        {
            if(a[i][j]==1) printf("X");
            else if(a[i][j]==2) printf("0");
            else printf(" ");
            if(j<n) printf("\t|\t");
        }
        printf("\n");
        if(i<n) printf("_______________________________________");
        printf("\n\n");
    }
}

void print_win_cells(int a[][4],int n,int win)
{
    printf("\n");
    for(int i=1; i<=n; i++)
    {
        for(int j=1; j<=n; j++)
        {
            if(a[i][j] == win)
            {
                if(a[i][j]==1) printf("X");
                else printf("0");
            }
            else printf(" ");
            if(j<n) printf("\t|\t");
        }
        printf("\n");
        if(i<n) printf("_______________________________________");
        printf("\n\n");
    }
}
int main()
{
    int n=3;
    int a[4][4];
    for(int i=1; i<=n; i++)
    {
        for(int j=1; j<=n; j++)
        {
            a[i][j]=-1;
        }
    }
    bool player1 = true;
    bool player2 = false;

    while(true)
    {
        print_cells(a,n);

        if(player1)
        {
            int r,c;
Turn1:
            printf("\nPlayer 1,Turn (X), Enter row and column: ");
            scanf("%d %d",&r,&c);
            if(a[r][c]!=-1)
            {
                printf("Invalid Cell!\n");
                goto Turn1;
            }
            a[r][c]=1;
            player1=false;
            player2=true;
        }
        else
        {
            int r,c;
Turn2:
            printf("\nPlayer 2,Turn (0), Enter row and column: ");
            scanf("%d %d",&r,&c);
            if(a[r][c]!=-1)
            {
                printf("Invalid Cell!\n");
                goto Turn2;
            }
            a[r][c]=2;
            player2=false;
            player1=true;
        }

        if(isWin(a,n)==1)
        {
            printf("\nPlayer 1 wins.\n");
            print_win_cells(a,n,1);
            break;
        }
        else if(isWin(a,n)==2)
        {
            printf("\nPlayer 2 wins.\n");
            print_win_cells(a,n,2);
            break;
        }

    }
    return 0;

}
